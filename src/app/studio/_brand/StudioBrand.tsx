 ...
 * - Jubilat serif for body, form labels, table cells, buttons and nav
 * - RCABLK display reserved for the wordmark, eyebrow accents, and the few
 *   genuine display moments (e.g. the big dashboard count). Used sparingly
 *   so it reads as a brand voice, not a UI font.
  ...
  return (
    <header className="flex flex-col gap-5 border-b border-black/10 pb-6 sm:gap-6 sm:pb-8 sm:flex-row sm:items-end sm:justify-between">
      <div className="flex min-w-0 max-w-2xl flex-col gap-3">
        ...
        {description && (
          <p className="font-serif text-[0.92rem] leading-relaxed text-black/65 sm:text-[1.05rem]">
            {description}
          </p>
        )}
      </div>
...
/**
 * Input base. The 16px (`text-base`) sizing on phones is intentional — iOS
 * Safari zooms into any input below 16px, so any narrower size triggers an
